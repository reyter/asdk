...
LRESULT CTextProgressCtrl::OnSetText(CString strType, LPCTSTR szText)
{
	...
	if((!strType && m_strType.GetLength()) || (strType && (m_strType != strType)))
		m_strType = strType;
...
void CTextProgressCtrl::OnPaint() 
{
    ...
    {
        CString str;
        if (m_strText.GetLength() && Fraction == 0)
            str = m_strText;
        else
            str.Format("%s：已完成%d%%", m_strType,(int)(Fraction*100.0));
