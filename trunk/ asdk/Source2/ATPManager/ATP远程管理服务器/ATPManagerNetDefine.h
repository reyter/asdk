...
struct	struAns_Operate
{
	...
	UINT8	nResult;				//	返回结果 TRUE成功  其他查表
};
...
struct struReq_ATPOperateLog_QUERY
{
	int		nBeginDate;
	int		nEndDate;
	int		nOperateType;
...
//! 查看算法组
//ID_ATPMANAGE_QUERYALGORITHMGROUPINFO			--------查看所有算法组，无其他条件查询



//算法参数
struct stru_ATPDataBaseAogorithmParam
{
...
//算法参数附加数据
struct stru_ATPDataBaseAogorithmParamEX
{
	UINT32	nSerialNumber;									//	自动增长 唯一
	int		nExtraDataLength;								//	附加数据长度	
	BYTE    ExtraData[512];
...
struct stru_ATPAlgorithmInfo
{
	...
	char				szMainDllName[32];
	char				szPath[MAX_PATH];				//	算法使用路径,路径要指定
	stru_BlockHead		AlgorithmParamBlock;		//	算法参数个数  算法参数
	stru_BlockHead		AlgorithmParamEXBlock;
	UINT8				nDeleteMark;				//	删除标志
};
...
struct struReq_ATPAlgorithm_QUERY
{
