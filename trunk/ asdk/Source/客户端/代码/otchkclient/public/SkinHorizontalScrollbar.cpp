...
void CSkinHorizontalScrollbar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	int nWidth = clientRect.Width()-17;

	CRect rectLeftArrow(0,0,17,17);
	CRect rectRightArrow(nWidth,0,nWidth+17,17);
	CRect rectThumb(nThumbLeft,0,nThumbLeft+41,17);
...
void CSkinHorizontalScrollbar::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if(bDragging)
	{	
		nThumbLeft = point.x-21; //-13 so mouse is in middle of thumb
		...
		int nScrollTimes = (int)((nThumbLeft-17)/dbThumbInterval)-nPos;
...
void CSkinHorizontalScrollbar::Draw()
{
	...
	CBitmap bitmap;
	//bitmap.LoadBitmap(IDB_H_LEFTARROW);
	bitmap.Attach(gpSkinManager->GetBitmap(H_LEFT));
	CBitmap* pOldBitmap = bitmapDC.SelectObject(&bitmap);

	memDC.BitBlt(clientRect.left,clientRect.top,17,17,&bitmapDC,0,0,SRCCOPY);
	...
	//bitmap.LoadBitmap(IDB_H_BACK);
	bitmap.Attach(gpSkinManager->GetBitmap(H_BACK));
	pOldBitmap = bitmapDC.SelectObject(&bitmap);
	

	int nWidth = clientRect.Width() - 17;

	memDC.StretchBlt(clientRect.left+17, clientRect.top, nWidth,17,&bitmapDC, 0,0, 1, 17, SRCCOPY);
	...
	//bitmap.LoadBitmap(IDB_H_RIGHTARROW);
	bitmap.Attach(gpSkinManager->GetBitmap(H_RIGHT));
	pOldBitmap = bitmapDC.SelectObject(&bitmap);
	memDC.BitBlt(nWidth,clientRect.top,17,17,&bitmapDC,0,0,SRCCOPY);
	...
	{
		//bitmap.LoadBitmap(IDB_H_MOVEBLOCK);
		bitmap.Attach(gpSkinManager->GetBitmap(H_THUMB));
		pOldBitmap = bitmapDC.SelectObject(&bitmap);
		memDC.BitBlt(clientRect.left+nThumbLeft,clientRect.top,41,17,&bitmapDC,0,0,SRCCOPY);
...
void CSkinHorizontalScrollbar::LimitThumbPosition()
{
	...
	if(nThumbLeft+41 > (clientRect.Width()-17))
	{
		nThumbLeft = clientRect.Width()-58;
	}

	if(nThumbLeft < (clientRect.left+17))
	{
		nThumbLeft = clientRect.left+17;
	}
}
