...
{
	if (zDelta < 0)
		m_SkinVerticleScrollbar.ScrollDown();//UpdateThumbPosition();
	else
		m_SkinVerticleScrollbar.ScrollUp();
...
void CSkinListCtrl::PositionScrollBars()
{
	...
	CRect rectHead;
	this->GetHeaderCtrl()->GetItemRect(0,rectHead);
	...
	rectHead.bottom = rectHead.top + 1;
	windowRect.top+=nTitleBarHeight+nDialogFrameHeight + rectHead.Height();
	...
	bV = false;
	bH = false;
	if (m_SkinVerticleScrollbar.IsWindowVisible())
		bV = true;
	if (m_SkinHorizontalScrollbar.IsWindowVisible())
		bH = true;
	CRect vBar(windowRect.right-nDialogFrameWidth,windowRect.top-nTitleBarHeight-nDialogFrameHeight,windowRect.right+17-nDialogFrameWidth,windowRect.bottom+17-nTitleBarHeight-nDialogFrameHeight);
	CRect hBar(windowRect.left-nDialogFrameWidth,windowRect.bottom-nTitleBarHeight-nDialogFrameHeight,windowRect.right+1-nDialogFrameWidth,windowRect.bottom+17-nTitleBarHeight-nDialogFrameHeight);
...
void CSkinListCtrl::ShowScrollBar(bool c_bV,bool c_bH)
{
	if (c_bV)
	...
	if (c_bH)
		m_SkinHorizontalScrollbar.ShowWindow(SW_SHOW);
	else
