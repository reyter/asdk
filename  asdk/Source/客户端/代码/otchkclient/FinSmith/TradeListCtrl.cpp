...
IMPLEMENT_DYNAMIC(CTradeListCtrl, CSkinListCtrl)
static int CALLBACK CompareFunction( LPARAM lParam1, LPARAM lParam2, LPARAM lParamData )
{
    CTradeListCtrl* pV=(CTradeListCtrl*)lParamData; 
...
BEGIN_MESSAGE_MAP(CTradeListCtrl, CSkinListCtrl)
	ON_WM_ERASEBKGND()
	...
	ON_WM_NCCALCSIZE()
	ON_NOTIFY_REFLECT(NM_CLICK, &CTradeListCtrl::OnNMClick)
END_MESSAGE_MAP()

void CTradeListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
///-	HGDIOBJ hobj;// = pDC->SelectObject(GetStockObject(DEFAULT_GUI_FONT));
	...
	{   
		//写文本   
		CString   szText;   
		SVirTrade *pItem = (SVirTrade *)GetItemData(lpDIS->itemID);	
		...
		{   //循环得到文本   
			...
			{
				if (i == 0 && m_bShowCheckBox)
				{
					if (GetCheck(lpDIS->itemID) == TRUE)
						pDC->DrawFrameControl(CRect(rcItem.left,rcItem.top,rcItem.left + 16,rcItem.bottom),DFC_BUTTON,DFCS_CHECKED);
					else
						pDC->DrawFrameControl(CRect(rcItem.left,rcItem.top,rcItem.left + 16,rcItem.bottom),DFC_BUTTON,DFCS_BUTTONCHECK);
					rcItem.left += 18;
					pDC->DrawText(szText,   lstrlen(szText),   &rcItem,   DT_BOTTOM|DT_LEFT|DT_NOPREFIX|DT_SINGLELINE); 
				}
				else
				if(i== 0 || i == 1)
				{					
...
BOOL CTradeListCtrl::OnEraseBkgnd(CDC* pDC)
{
...
void CTradeListCtrl::OnColumnClick( NMHDR* pNMHDR, LRESULT* pResult )
{
...
void CTradeListCtrl::PreSubclassWindow()
{
...
////////////////////////////////////////////
int CTradeListCtrl::OnToolHitTest(CPoint point, TOOLINFO * pTI) const
{
	int row, col;
	RECT cellrect;
...
int CTradeListCtrl::CellRectFromPoint(CPoint & point, RECT * cellrect, int * col) const
{
	int colnum;
	CHeaderCtrl* pHeader;
	int nColumnCount;
...
//////////////////////////////////////////////////
BOOL CTradeListCtrl::OnToolTipText( UINT id, NMHDR * pNMHDR, LRESULT * pResult )
{
...
void CTradeListCtrl::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CSkinListCtrl::OnNcCalcSize(bCalcValidRects, lpncsp);
}

void CTradeListCtrl::OnNMClick(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 在此添加控件通知处理程序代码
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*)pNMHDR;
