...
{
	m_nHidedCol = -1;
	m_cxSplitter = m_cySplitter = 1 + 1 + 1;
	m_cxBorderShare = m_cyBorderShare = 0;
	m_cxSplitterGap = 1 + 1 + 1;// + 5;
	m_cySplitterGap = 1 + 1 + 1;// + 5;
...
void CFlatSplitterWnd::OnDrawSplitter(CDC* pDC, ESplitType nType, const CRect& rectArg)
{
	...
	//pDC->Draw3dRect(rectArg, GetSysColor(COLOR_WINDOW), GetSysColor(COLOR_BTNHIGHLIGHT));
}
...
void CFlatSplitterWnd::ShowColumn()
{
   ASSERT_VALID(this);
...
void CFlatSplitterWnd::HideColumn(int colHide)
{
   ASSERT_VALID(this);
   ASSERT(m_nCols > 1);
   ASSERT(colHide < m_nCols);
   ASSERT(m_nHidedCol == -1);
   if (m_nHidedCol == colHide)
	   return;
   m_nHidedCol = colHide;
...
void CFlatSplitterWnd::OnSize(UINT nType, int cx, int cy)
{
	CSplitterWnd::OnSize(nType, cx, cy);

	// TODO: 在此处添加消息处理程序代码
	//this->GetParentFrame()->Invalidate(0);
	/*
	CRect rect;
	this->GetParentFrame()->GetClientRect(&rect);
	rect.left = cx;
	rect.right = cx+10;
	this->GetParentFrame()->InvalidateRect(rect,0);
	*/
}
