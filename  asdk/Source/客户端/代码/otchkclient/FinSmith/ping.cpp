...
BOOL CPing::Initialise1() const
{
	if (!sm_bAttemptedIcmpInitialise)
	{
		...
		_cpingpData->sm_hIcmp = LoadLibrary(_T("ICMP.DLL"));
		if (_cpingpData->sm_hIcmp == NULL)
		{
		...
		sm_pIcmpCreateFile = (lpIcmpCreateFile) GetProcAddress(_cpingpData->sm_hIcmp,"IcmpCreateFile");
		sm_pIcmpSendEcho = (lpIcmpSendEcho) GetProcAddress(_cpingpData->sm_hIcmp,"IcmpSendEcho" );
		sm_pIcmpCloseHandle = (lpIcmpCloseHandle) GetProcAddress(_cpingpData->sm_hIcmp,"IcmpCloseHandle");
...
BOOL CPing::Ping1(LPCTSTR pszHostName, CPingReply& pr, UCHAR nTTL, DWORD dwTimeout, UCHAR nPacketSize) const
{
  ...
  ASSERT(_cpingpData->sm_hIcmp); //ICMP dll must be open
