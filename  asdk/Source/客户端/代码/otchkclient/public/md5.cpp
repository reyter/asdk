...
/*  
MD5_Init initializes the message-digest context mdContext.  
...
/*  
MD5_Update updates the message-digest context to 
account for the presence of each of the characters inBuf[0..inLen-1] 
in the message whose digest is being computed.   
*/ 
void MD5_Update ( MD5_CTX *mdContext, unsigned char *inBuf, unsigned 
int inLen ) 
{ 
    UINT4        in[16]; 
    int          mdi; 
...
#else 
    if (((mdContext->i[0]+((UINT4)inLen << 3)) & 0xffffffff) < 
mdContext->i[0]) 
#endif 
...
/*  
MD5_Final terminates the message-digest computation and 
 ...
 * 
 */ 
char *btob64 ( register unsigned char *pcbitvec, register char *pcout, 
int numbits ) 
{ 
...
int MD5_Sig_Get (unsigned char * pczSrc,int iSrcLen,char * pczSig,int iSigLen)
{
    ...
    iReadIn = 0;
    buffer = pczSrc;
    while (iReadIn < iSrcLen)
    {
        if (iSrcLen > iReadIn + BUFSIZE)
		{
       
      		MD5_Update( mdbuf, buffer + iReadIn, BUFSIZE);
        }
        else
        {
			MD5_Update( mdbuf, buffer + iReadIn, iSrcLen - iReadIn);
        }
 		iReadIn += BUFSIZE;
    }
    ...
    if ( printhex )
    {
		printf("printhex\n");
 		for ( i = 0; i < 16; i++ )
		{
     		sprintf_s( s, "%02lX", mdbuf->digest[i] );
     		strcat(pczSig, s);
        }  /* end of for */
    }
    /* base 64 */
    ...
    {
 		btob64( mdbuf->digest, pczSig, 128 );
    }
