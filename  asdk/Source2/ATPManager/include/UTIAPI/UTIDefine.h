...
struct UTI_GroupInfo
{
	char chGroupCode[256];				//所含有效营业部,  营业部代码间用'|'隔开 如:123|234|222|
	char chGroupName[48];				//组名
	char chResv[16];					//保留

	UTI_GroupInfo()
	{
		memset(this, 0, sizeof(UTI_GroupInfo));
	}
};
...
*	交易传递信息
*/
struct _struWtInfoExt
{
	int  type;              //信息类型   0--委托信息　
	char StkCode[12];
	int  exch_id;
	char sResult[100];		//返回内容
};
