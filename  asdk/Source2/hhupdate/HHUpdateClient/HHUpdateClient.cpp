// HHUpdateClient.cpp : 定义应用程序的类行为。
//

#include "stdafx.h"
#include "HHUpdateClient.h"
#include "HHUpdateClientDlg.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif


// CHHUpdateClientApp

BEGIN_MESSAGE_MAP(CHHUpdateClientApp, CWinApp)
	ON_COMMAND(ID_HELP, CWinApp::OnHelp)
END_MESSAGE_MAP()


// CHHUpdateClientApp 构造

CHHUpdateClientApp::CHHUpdateClientApp()
{
	// TODO: 在此处添加构造代码，
	// 将所有重要的初始化放置在 InitInstance 中
}


// 唯一的一个 CHHUpdateClientApp 对象

CHHUpdateClientApp theApp;


// CHHUpdateClientApp 初始化

BOOL CHHUpdateClientApp::InitInstance()
{
	if (!FirstInstance())
	{
		AfxMessageBox("程序已经运行");
		return FALSE;	//已经有实例存在了，退出
	}
		

	// 如果一个运行在 Windows XP 上的应用程序清单指定要
	// 使用 ComCtl32.dll 版本 6 或更高版本来启用可视化方式，
	//则需要 InitCommonControls()。否则，将无法创建窗口。
	InitCommonControls();

	CWinApp::InitInstance();

	if (!AfxSocketInit())
	{
		AfxMessageBox(IDP_SOCKETS_INIT_FAILED);
		return FALSE;
	}

	AfxEnableControlContainer();
	AfxInitRichEdit2();
	// 标准初始化
	// 如果未使用这些功能并希望减小
	// 最终可执行文件的大小，则应移除下列
	// 不需要的特定初始化例程
	// 更改用于存储设置的注册表项
	// TODO: 应适当修改该字符串，
	// 例如修改为公司或组织名
	SetRegistryKey(_T("应用程序向导生成的本地应用程序"));

	CHHUpdateClientDlg dlg;
	m_pMainWnd = &dlg;
	INT_PTR nResponse = dlg.DoModal();
	if (nResponse == IDOK)
	{
		// TODO: 在此放置处理何时用“确定”来关闭
		//对话框的代码
	}
	else if (nResponse == IDCANCEL)
	{
		// TODO: 在此放置处理何时用“取消”来关闭
		//对话框的代码
	}

	// 由于对话框已关闭，所以将返回 FALSE 以便退出应用程序，
	// 而不是启动应用程序的消息泵。
	return FALSE;
}

BOOL CHHUpdateClientApp::FirstInstance()
{
	CWnd *pWndPrev, *pWndChild; 
	//根据主窗口类名和主窗口名判断是否已经有实例存在了
	if (pWndPrev = CWnd::FindWindow(_T("#32770"),"宏汇NSD自动升级服务"))//#32770是默认的对话框类名字
	{
		//如果存在就将其激活，并显示出来	  
		pWndChild = pWndPrev->GetLastActivePopup(); 	  
		//如果是最小化的就还原窗口
		if (pWndPrev->IsIconic()) 
			pWndPrev->ShowWindow(SW_RESTORE);      	  
		pWndChild->SetForegroundWindow();         	 
		return FALSE;                             
	}
	else
		return TRUE;                             
}
